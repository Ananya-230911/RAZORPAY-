...
from app.common import load_results, simulated_banner  # noqa: E402
...
with col_btn:
    if st.button("↻ Re-run evaluation (reconciliation + classifier only, free & instant)"):
        with st.spinner("Re-running matcher + classifier against ground truth..."):
            results = run_evaluation()
        st.success("Done — evaluation/results.json updated.")
st.caption(
    "This button only re-runs the deterministic part (Module 2+3+9). To refresh the AI-resolution "
...
st.subheader("Exception classification (Module 3)")
cols = st.columns(2)
